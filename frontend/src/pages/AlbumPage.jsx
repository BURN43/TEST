...
const AlbumPage = ({ isGuest }) => {
  ...
  const handleMediaUpload = (e) => {
    ...
    .then(response => {
      ...
      // Replace the temporary media with the actual media
      setMedia((prevMedia) => prevMedia.map((item) => (item.id === tempId ? newMedia : item)));
      setLoading(false);
    })
    .catch(error => {
      console.error('Error uploading media:', error);
      // Remove the temporary media if the upload fails
  ...
  const renderMedia = (mediaItem) => {
    if (!mediaItem || !mediaItem.mediaUrl) {
      // Check if mediaItem has a valid URL
      return <div className="text-red-500">Invalid media item</div>;
    }
  
    // Ensure the media URL is fully qualified by adding the base URL
    const baseURL = 'http://localhost:5000';
      ...
      >
        ...
        <div className="grid grid-cols-3 gap-1 md:grid-cols-6 h-fit">
          ...
          {media.length > 0 ? (
            media.map((mediaItem) => (
              <motion.div
                key={mediaItem._id || mediaItem.id} // Ensure that mediaItem._id or tempId is used as the key
